...
$.fn.tree = function(settings){
  ...
  return $(this).each(function(){
    ...
    if ( typeof Drupal != "undefined" )
        window.themePath = Drupal.settings.basePath + Drupal.settings.pathToTheme;
    ...
    tree.find('li:has(ul)')
        ...
        .before('<button type="button" class="tree-button" title="Déplier"></button>');
    ...
    tree
      ...
      .bind('expand',function(event){
        ...
        target.next().hide().removeClass('tree-group-collapsed').slideDown(150, function(){
          ...
          target.parent().attr('aria-expanded', 'true').addClass('tree-expanded');
        });
      })
      ...
      .bind('collapse',function(event){
        ...
        target.next().slideUp(150, function(){
          target.parent().attr('aria-expanded', 'false').removeClass('tree-expanded');
    ...
    tree
      ...
      .click(function(event){
        ...
        if( target.is('button, img') ){
          target.closest('li').find('> a').trigger('toggle');
          target.closest('li').find('> a').eq(0).focus();
